...
ratings <- read.csv("ratings1M.csv", header = TRUE)
...
ratingmat <- as.matrix(ratingmat, "realRatingMatrix")
...
recommender_model <-
  Recommender(ratingmat_norm,
              method = "UBCF",
              param = list(method = "Cosine", nn = 30))
recom <-
  predict(recommender_model, ratingmat[1], n = 10) #Obtain top 10 recommendations for 1st user in dataset
recom_list <-
  as(recom, "list") #convert recommenderlab object to readable list
...
algorithms <- list(
  ...
  "user-based CF" = list(name = "UBCF", param = list(method = "Cosine", nn =
                                                       30))
)

evaluation_results <-
  evaluate(evaluation_scheme, algorithms, n = c(1, 3, 5, 10, 15, 20)) #n=c denote top-N
